...
public class YDLHUD:UIView {
    
    private var delay : TimeInterval = delayTime
    private var imageView :UIImageView?
    private var activityView : UIActivityIndicatorView?
    private var type : YDLHUDType?
    private var text : String?
    private var selfWidth:CGFloat = 90
    private var selfHeight:CGFloat = 90
    
    // enable ：是否允许用户交互，默认允许。
    init(text:String?,type:YDLHUDType,delay:TimeInterval,enable:Bool = true,offset:CGPoint = CGPoint(x: 0, y: -50)) {
        self.delay = delay
        self.text = text
        self.type = type
    ...
    private func setupUI() {
        ...
        switch type {
        case .success:
            addImageView(image: YDLHUDImage.imageOfSuccess)
        case .error:
            addImageView(image: YDLHUDImage.imageOfError)
        case .info:
            addImageView(image: YDLHUDImage.imageOfInfo)
        case .loading:
            addActivityView()
        case .text:
            break
    ...
    public static func showSuccess(text:String?,delay:TimeInterval,enable:Bool = true) {
        YDLHUD(text: text, type: .success, delay: delay,enable: enable).show()
    }
    
    public static func showError(text:String?,delay:TimeInterval,enable:Bool = true) {
        YDLHUD(text: text, type: .error, delay: delay,enable:enable).show()
    }
    
    public static func showLoading(enable:Bool = false) {
        YDLHUD(text: nil,type:.loading,delay: 0,enable:enable).show()
    }
    
    public static func showLoading(text:String?,enable:Bool = true) {
        YDLHUD(text: text,type:.loading,delay: 0,enable:enable).show()
    }
    
    public static func showInfo(text:String?,delay:TimeInterval,enable:Bool = true) {
        YDLHUD(text: text, type: .info, delay: delay,enable:enable).show()
    }
    
    public static func showText(text:String?,delay:TimeInterval,enable:Bool = true) {
        YDLHUD(text: text,type:.text,delay: delay,enable:enable).show()
    }
    ...
    public func show() {
        ...
        self.animate(hide: false) {
            if self.delay > 0 {
                YDLHUD.asyncAfter(duration: self.delay, completion: {
                    self.hide()
                })
            }
        }
    ...
    public func hide(delay:TimeInterval) {
        YDLHUD.asyncAfter(duration: delay) {
            self.hide()
        }
    }
...
//MARK: Class YDLHUDImage
private class YDLHUDImage {
    ...
    fileprivate class func draw(_ type: YDLHUDType) {
